...
//#region node_modules/@sapphire/snowflake/dist/esm/index.mjs
...
var TimestampFieldDivisor = 4194304;
var _a, _b, _c, _d, _e;
...
//#endregion
//#region plugins/test-commands-api/index.ts
const { plugin: { scoped, store }, flux: { storesFlat }, util: { log } } = shelter;
...
function onLoad() {
	...
	scoped.flux.intercept((payload) => {
		...
		for (const command of commands) index.application_commands.push(command);
	});
	registerCommand({
		name: "aaaa",
		description: ""
...
function onUnload() {
	...
	const currentChannel = storesFlat.ChannelStore.getChannel(storesFlat.SelectedChannelStore.getChannelId());
	shelter.flux.dispatcher.dispatch({
		type: "APPLICATION_COMMAND_INDEX_FETCH_REQUEST",
		target: currentChannel.guild_id === null ? {
			type: "channel",
